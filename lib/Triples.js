...
function Triples (triples) {
  ...
  for ( var i = 0 ; i < triples.length ; i ++) {
    if( !( (N3Util.isBlank(triples[i].subject)) && (triples[i].object == 'http://www.w3.org/2002/07/owl#Class') ) )
...
Triples.prototype.getUrisOfType = function (type) {
  ...
  for (var i = 0; i < type.length; i++) {
    var uris = this.getTriples( { predicate: "http://www.w3.org/1999/02/22-rdf-syntax-ns#type" , object : type[i] } ).all;
